...
class HeterEventDispatcherBase {
 protected:
  ...
  using MixinRoot =
      typename std::conditional<std::is_same<MixinRoot_, void>::value, ThisType,
                                MixinRoot_>::type;
  ...
  static_assert(
      ...
      "ArgumentPassingMode can't be ArgumentPassingAutoDetect in heterogeneous "
      "dispatcher.");
 ...
 public:
  ...
  void directDispatch(const Event &e, Args &&... args) const {
    if (!internal_::ForEachMixins<MixinRoot, Mixins, DoMixinBeforeDispatch>::
            forEach(this,
 ...
 protected:
  ...
  auto doDispatch(T &&first, Args &&... args) const -> typename std::enable_if<
    ...
    if (!internal_::ForEachMixins<MixinRoot, Mixins, DoMixinBeforeDispatch>::
            forEach(this, typename std::add_lvalue_reference<T>::type(first),
                    typename std::add_lvalue_reference<Args>::type(args)...)) {
      return;
    }
 ...
 private:
  ...
  struct DoMixinBeforeDispatch {
    ...
    static auto forEach(const Self *self, A &&... args) ->
        typename std::enable_if<HasFunctionMixinBeforeDispatch<T, A...>::value,
                                bool>::type {
