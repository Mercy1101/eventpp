...
struct IntToConstantHelper {
  ...
  static auto find(const int index, C&& c, Args&&... args) -> decltype(
      std::declval<C>().template operator()<0>(std::declval<Args>()...)) {
    if (N == index) {
      return c.template operator()<N>(std::forward<Args>(args)...);
    } else {
      return IntToConstantHelper<N + 1, M>::find(index, std::forward<C>(c),
...
auto intToConstant(const int index, C&& c, Args&&... args) -> decltype(
    std::declval<C>().template operator()<0>(std::declval<Args>()...)) {
