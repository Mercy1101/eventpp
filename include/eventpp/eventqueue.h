 ...
 private:
  ...
  struct QueuedEvent_ {
    typename std::remove_cv<
        typename std::remove_reference<typename super::Event>::type>::type
        event;
 ...
 public:
      ...
      if (!tempList.empty()) {
        for (auto it = tempList.begin(); it != tempList.end();) {
          if (doInvokeFuncWithQueuedEvent(
                  func, it->template get<QueuedEvent_>(),
                  typename MakeIndexSequence<sizeof...(Args)>::Type())) {
            doDispatchQueuedEvent(
  ...
  bool waitFor(const std::chrono::duration<Rep, Period> &duration) const {
    ...
    return queueListConditionVariable.wait_for(
        queueListLock, duration, [this]() -> bool { return doCanProcess(); });
  }
  ...
  auto dispatch(const U &queuedEvent) ->
    ...
    doDispatchQueuedEvent(queuedEvent,
                          typename MakeIndexSequence<sizeof...(Args)>::Type());
  }
 ...
 protected:
  ...
  bool doInvokeFuncWithQueuedEvent(F &&func, T &&item,
    ...
    return doInvokeFuncWithQueuedEventHelper(
        std::forward<F>(func), item.event,
        std::get<Indexes>(item.arguments)...);
  }

  template <typename F>
  bool doInvokeFuncWithQueuedEventHelper(F &&func,
                                         const typename super::Event & /*e*/,
                                         Args... args) const {
